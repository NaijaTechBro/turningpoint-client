    ...
    <div className="space-y-6">
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
        ...
        <div className="bg-white p-3 rounded-2xl border border-gray-100 flex items-center gap-3 shadow-sm w-full md:w-96">
          ...
          <input 
            type="text"
            placeholder="Search Name, Lab Ref, Test..."
            className="flex-1 outline-none font-medium text-brand-blue text-sm w-full"
            value={searchTerm}
            onChange={(e) => setSearchTerm(e.target.value)}
          />
        </div>
      ...
      {orders.length === 0 ? (
        ...
        <div className="bg-white rounded-[2.5rem] shadow-sm border border-gray-100 overflow-hidden">
          {/* NEW: Added overflow-x-auto wrapper to make the table scrollable horizontally on small screens */}
          <div className="overflow-x-auto w-full">
            {/* NEW: Added min-w-[1000px] so the table refuses to shrink past the point where it hides buttons */}
                  ...
                  filteredOrders.map(order => {
                    ...
                    return (
                      <tr key={order._id} className="hover:bg-blue-50/30 transition-colors group">
                        ...
                        <td className="px-8 py-6">
                          <span className="font-bold text-brand-blue line-clamp-2">{order.template?.testName || 'Unknown / Deleted Test'}</span>
                        </td>
                            ...
                            {(order.status === 'VERIFIED' || order.status === 'DELIVERED') ? (
                              <>
                                ...
                                <button 
                                  onClick={() => handleViewPDF(order._id, order.labReference)}
                                  disabled={actionLoading.id === order._id}
                                  className="px-3 py-2 bg-brand-blue text-white rounded-lg text-xs font-bold hover:bg-blue-900 transition-colors flex items-center gap-2 disabled:opacity-50 shrink-0"
                                  title="View PDF"
                                >
                                  {actionLoading.id === order._id && actionLoading.action === 'pdf' 
                                    ? <Loader2 size={14} className="animate-spin"/> 
                                ...
                                <button 
                                  ...
                                  disabled={actionLoading.id === order._id}
                                  title={order.patient?.email ? "Send via Email" : "No Email Provided"}
                                  className={`px-3 py-2 rounded-lg text-xs font-bold transition-colors flex items-center gap-2 disabled:opacity-50 shrink-0 ${
                                    order.patient?.email 
                                      ? 'bg-orange-100 text-brand-orange hover:bg-brand-orange hover:text-white' 
                                      : 'bg-gray-100 text-gray-400 opacity-60 cursor-not-allowed'
                                  }`}
