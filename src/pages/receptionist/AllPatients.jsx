  ...
  return (
    <div className="space-y-6">
      ...
      <div className="bg-white p-4 rounded-2xl border border-gray-100 flex items-center gap-3 shadow-sm">
        ...
        <input 
          type="text"
          placeholder="Filter by Name, Laboratory ID, or Phone..."
          ...
          <>
            <table className="w-full text-left">
              <thead className="bg-gray-50 border-b border-gray-100">
                <tr>
                  {/* CHANGED TO LABORATORY ID */}
                  <th className="px-8 py-5 text-[10px] font-black text-gray-400 uppercase tracking-widest">Laboratory ID</th>
                  ...
                  patients.map((p) => (
                    <tr key={p._id} className="hover:bg-gray-50/50 transition-colors group">
                      ...
                      <td className="px-8 py-6 text-right space-x-2">
                        <button onClick={() => navigate(`/receptionist/patient-details/${p._id}`)} className="p-2 hover:bg-gray-100 text-gray-400 hover:text-brand-blue rounded-lg transition-colors" title="View Profile">
                          <UserCircle size={20} />
                        </button>
                        <button onClick={() => navigate(`/receptionist/edit-patient/${p._id}`)} className="p-2 hover:bg-gray-100 text-gray-400 hover:text-brand-orange rounded-lg transition-colors" title="Edit Details">
                          <Edit size={20} />
                        </button>
                        <button onClick={() => navigate(`/receptionist/order-test/${p._id}`)} className="ml-2 px-4 py-2 bg-brand-blue text-white text-[10px] font-black uppercase tracking-widest rounded-lg hover:bg-blue-900 transition-all">
                          Order Test
                        </button>
                      </td>
                    </tr>
            ...
            <div className="p-6 border-t border-gray-100 flex justify-between items-center bg-gray-50/30">
              ...
              <div className="flex gap-2">
                <button disabled={page === 1} onClick={() => setPage(p => p - 1)} className="p-2 border bg-white rounded-lg hover:bg-gray-50 disabled:opacity-30 transition-all"><ChevronLeft size={20}/></button>
                <button disabled={page === totalPages} onClick={() => setPage(p => p + 1)} className="p-2 border bg-white rounded-lg hover:bg-gray-50 disabled:opacity-30 transition-all"><ChevronRight size={20}/></button>
              </div>
            </div>
          </>
        )}
